...
if ($error == "none"){
	$sql = "UPDATE events 
			SET entity_id = ". addslashes($id) . ", 
				...
				timestamp = '". $timestamp. "',
				endtime = '". addslashes((isset($endtime))?$endtime:"") . "',
				number = ". addslashes((isset($number))?$number:"") . "
				WHERE event_id = ". addslashes($event_id) . "
				;";
	//echo $sql;
...
echo "'starttime': '" . addslashes($starttime) . "',\n";
echo "'endtime': '" . addslashes($endtime) . "',\n";
